...
void mochila(int nitens, int capacidadet, int nCasas, int cidade_inicial, int peso[], dado motoboy[], int nmotoboys, list<no>adj[])
{
	...
	for(j = 1; j <= nmotoboys; j++)
	{
		...
		for(i = 1; i <= nitens + 1; i++)
		{
			if(capacidader >= peso[i] && i != cidade_inicial && pega[i] == false)
			{
				k++;
				motoboy[j].itens[k] = i;
				capacidader -= peso[i];
		...
		if(k == 0)
		{
			cout << "Nao pegou nenhum item" << endl;
...
int main()
{
	...
	int casa_inical; // vértice inicial da árvore
	int i, j; // contadores
	...
	cout << "Entre com o número que pertence ao supermercado e aonde quer chegar" << endl;
	cin >> casa_inical;
	...
	mochila(nitens, capacidadet, nCasas, casa_inical, peso, motoboy, nmotoboys, adj);
